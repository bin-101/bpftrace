// bpffeature.cpp (解説付きコピー)
// src/bpffeature.cpp と同じコードに日本語コメントを挿入したもので、
// 実際のビルドには使用されません。各関数の役割やポイントを理解するための資料です。
...
// bpftrace 名前空間に各種検出ロジックが定義される。
namespace bpftrace {

using util::KernelVersionMethod;

// ------------------------------------------------------
// BPFnofeature::parse: 環境変数 NOFEATURE からユーザーが無効化したい機能を解析し、フラグを立てる。
...
// ------------------------------------------------------
// try_load_: 与えられた命令列で BPF プログラムを一度ロードしてみて、対応 helper / prog が使えるか調べる。
...
// ------------------------------------------------------
// BPFfeature::try_load: try_load_ をラップし、BTF 情報を利用できる場合には attach_btf_id も指定。
...
// ------------------------------------------------------
// BPFfeature::try_load_btf: BTF データがロードできるかテストし、結果を返す。
...
// ------------------------------------------------------
// detect_helper: 指定 helper を呼ぶだけの最小プログラムをロードし、成功すれば利用可能と判定。ログを解析して失敗理由も把握する。
...
// ------------------------------------------------------
// detect_prog_type: 指定したプログラムタイプをロードしてみて利用可否を調べる。helper 検出などでも利用。
...
// ------------------------------------------------------
// has_btf: BTF 情報が取得できるかキャッシュ付きで照会。
...
// ------------------------------------------------------
// instruction_limit: カーネルで設定されている BPF 命令数上限を取得。結果はキャッシュされる。
...
// ------------------------------------------------------
// has_map_batch: map のバッチ操作 helper が使えるかを調べる。
...
// ------------------------------------------------------
// has_d_path: BPF_FUNC_d_path が利用可能か試し、結果をキャッシュ。
...
// ------------------------------------------------------
// has_kprobe_multi: kprobe_multi 機能が使えるかを確認。無効化フラグが立っている場合は false。
...
// ------------------------------------------------------
// has_kprobe_session: kprobe session が利用可能か確認。
...
// ------------------------------------------------------
// has_uprobe_multi: uprobe_multi の機能が使えるかを確認する。
...
// ------------------------------------------------------
// report: 収集した helper / feature を文字列として整形し、`bpftrace --info` などで表示するために使う。
...
// ------------------------------------------------------
// has_prog_fentry: fentry/fexit プログラムが利用できるか検出する。
...
// ------------------------------------------------------
// has_iter: 指定された iterator プログラムが利用できるか検出する。
...
// ------------------------------------------------------
// has_kernel_func: BTF を使って指定されたカーネル関数が利用可能か確認する（結果はキャッシュされる）。
